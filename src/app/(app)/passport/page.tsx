  ...
  return (
    <div className="pb-24 md:pb-8">
      <div className="mb-5 flex items-start justify-between gap-3">
        <PageTitle title="תיק העסק" subtitle="כל העסק במקום אחד — לבנק, לרו״ח, ולשקט הנפשי שלכם" />
        <a
        ...
        <FadeIn>
          <Card elevated className="relative overflow-hidden p-5">
            ...
            <div className="relative flex flex-wrap items-center gap-4">
              <span className="flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br from-brand-600 to-brand-400 text-xl font-extrabold text-white shadow-e-brand">
                {d.levelNumber}
              </span>
              <div className="min-w-0 flex-1">
                <p className="text-xs font-semibold uppercase tracking-wide text-ink-muted">מצב מוכנות</p>
                <p className="text-gradient text-xl font-extrabold">{d.levelTitle}</p>
                <p className="tnum mt-0.5 text-sm text-ink-muted">ציון {d.score} · {d.completedCount}/{d.totalCount} משימות הושלמו</p>
              </div>
            </div>
            {d.badges.length > 0 && (
            ...
            <dl className="grid gap-x-6 gap-y-2 sm:grid-cols-2">
              {d.identity.map((i) => (
                <div key={i.label} className="flex items-center justify-between gap-3 border-b border-edge-soft py-1.5">
                  ...
                  <dd dir="ltr" className="text-sm font-semibold text-ink">{i.value}</dd>
                </div>
              ))}
            </dl>
          </PassportPanel>
