...
export async function getBusinessTasks(
  ...
  const { data, error } = await supabase
    .from("business_tasks")
    .select(
      "id, business_id, template_id, status, due_date, completed_at, notes, is_relevant, dismissal, dismissal_note, completion_data, steps_done, stage, follow_up_date, waiting_for"
    )
